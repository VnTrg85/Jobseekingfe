...
const RoleManager = () => {
	...
	useEffect(() => {
		const fetchRoles = async () => {
			try {
				const response = await axios.get("http://localhost:8080/userRole/getAll");
	...
	const searchFilter = () => {
		setFilteredRoles(roles.filter(role => role.name.toLowerCase().includes(filter.toLowerCase())));
	};
	...
	const updateRole = async id => {
		if (window.confirm("Do you want to save changes to this role?")) {
			await axios.put(`http://localhost:8080/userRole/update/${id}`, { name: editingRoleName });
	...
	const roleDelete = async id => {
		if (window.confirm("Are you sure you want to delete this role?")) {
			try {
				await axios.delete(`http://localhost:8080/userRole/delete/${id}`);
	...
	const addRole = async e => {
		...
		const response = await axios.post("http://localhost:8080/userRole/add", { name: newRoleName });
	...
	return (
		<div className="role-manager">
			...
			<form onSubmit={addRole} className="role-form">
				<input type="text" value={newRoleName} onChange={e => setNewRoleName(e.target.value)} placeholder="New Role Name" required />
