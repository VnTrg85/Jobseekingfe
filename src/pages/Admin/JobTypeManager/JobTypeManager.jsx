...
const JobTypeManager = () => {
	...
	useEffect(() => {
		const fetchJobTypes = async () => {
			try {
				const response = await axios.get("http://localhost:8080/jobtype/getAll");
	...
	const searchFilter = () => {
		setFilteredJobTypes(jobTypes.filter(jobType => jobType.name.toLowerCase().includes(filter.toLowerCase())));
	};
	...
	const jobTypeDelete = async id => {
		if (window.confirm("Are you sure you want to delete this job type?")) {
			try {
				await axios.delete(`http://localhost:8080/jobtype/${id}`);
	...
	const jobTypeUpdate = async e => {
		...
		if (window.confirm("Are you sure you want to update this job type?")) {
			try {
				await axios.put(`http://localhost:8080/jobtype/${editingJobTypeId}`, { name: editingJobTypeName });
				...
				setFilteredJobTypes(
					filteredJobTypes.map(jobType => (jobType.id === editingJobTypeId ? { ...jobType, name: editingJobTypeName } : jobType)),
				);
				jobTypeCancelEdit();
			} catch (error) {
	...
	const jobTypeAdd = async e => {
		...
		try {
			...
			const response = await axios.post("http://localhost:8080/jobtype/add", { name: newJobTypeName });
	...
	return (
		<div className="job-type-manager">
			...
			<input type="text" placeholder="Filter by name" value={filter} onChange={e => setFilter(e.target.value)} className="filter-input" />
			<button onClick={searchFilter} className="filter-button">
				Filter
			</button>
						...
						<tr key={jobType.id}>
							<td>
								{editingJobTypeId === jobType.id ? (
									<input type="text" value={editingJobTypeName} onChange={e => setEditingJobTypeName(e.target.value)} />
								) : (
									jobType.name
								)}
							</td>
							<td>
								{editingJobTypeId === jobType.id ? (
									<>
										<button onClick={jobTypeCancelEdit} className="filter-button">
											Cancel
										</button>
										<button onClick={jobTypeUpdate} className="filter-button">
											Save
										</button>
									</>
								) : (
									<>
										<button onClick={() => jobTypeEdit(jobType)} className="filter-button">
											Edit
										</button>
										<button onClick={() => jobTypeDelete(jobType.id)} className="filter-button">
											Delete
										</button>
									</>
								)}
